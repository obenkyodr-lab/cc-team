...
- ファイル削除の権限はClaudeに与えない。削除が必要なファイル・フォルダは `rm` を使わず `0_inbox/trash/` へ移動する（`settings.json` の deny で `rm`/`rmdir`/`unlink` 系コマンドをブロック済み）
- Gmail連携時、メール・スレッドの削除（delete）・ゴミ箱移動（trash）操作は行わない（`settings.json` の deny で該当ツールをブロック済み）。アーカイブ・既読化・ラベル付け・下書き作成は可
